 ...
 *
 * Starting with a copy of "class.dompdf_export.php 40821 2010-12-05 14:00:09Z"
 *
 *
 * Modifications by Martin Bless <martin.bless@gmail.com>
 *
 */
...
class pdfresult_dompdf {
	...
	public function getQuestions(){
		...
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_getQuestions'])){
			foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_getQuestions'] as $_classRef){
				$_procObj = & t3lib_div::getUserObj($_classRef);
				$hook_questions = $_procObj->pdfresult_dompdf_getQuestions($this);
	...
	function getOutcomes(){
		...
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_getOutcomes'])){
			foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_getOutcomes'] as $_classRef){
				$_procObj = & t3lib_div::getUserObj($_classRef);
				$hook_outcomes = $_procObj->pdfresult_dompdf_getOutcomes($this);
	...
	function getTemplates(){
		...
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_getTemplates'])){
			foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_getTemplates'] as $_classRef){
				$_procObj = & t3lib_div::getUserObj($_classRef);
				$this->templates = $_procObj->pdfresult_dompdf_getTemplates($this,$templateFolder,$this->templates);
			}
		}
							...
							foreach ($subquestions as $sub){
								if ($question['matrix_pointsforcolumn'] == 1){
									foreach ($columns as $col){
										...
										if ($result[$qid]['answer']['options'][$sub['uid']][$col['uid']][0] == $col['uid']){
											$bars['own'][$qid] += intval($sub['value']);
										}
									}
									//$bars['own'][$qid] += intval($question_obj->subquestions[$this->piVars[$qid]['options'][$sub['uid']]['single']]['value']);
									...
									if (is_array($result[$qid]['answer']['options'][$sub['uid']])){
										foreach ($result[$qid]['answer']['options'][$sub['uid']] as $r_item){
											$bars['own'][$qid] += intval($columns[$r_item[0]]['value']);
										}
									}
								}
							}
	...
	public function renderQuestion($question, $compare=false) {
		...
		switch ($question['type']){
			...
			case 'privacy':
				...
				break;
			default:
				if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_renderQuestion'])){
					foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ke_questionnaire']['pdfresult_dompdf_renderQuestion'] as $_classRef){
						$_procObj = & t3lib_div::getUserObj($_classRef);
						$html = $_procObj->pdfresult_dompdf_renderQuestion($this,$markerArray,$question, $answered);
					}
				}
		}
		//$html .= '</div>';
